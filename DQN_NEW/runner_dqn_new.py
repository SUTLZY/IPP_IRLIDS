...
from worker_dqn_new import Worker
...
class Runner(object):
    ...
    def singleThreadedJob(self, episodeNumber, budget_range, sample_size, sample_length, random):
        save_img = True if (SAVE_IMG_GAP != 0 and episodeNumber % SAVE_IMG_GAP == 0) else False
        #save_img = False
        worker = Worker(self.metaAgentID, self.localNetwork, episodeNumber, budget_range, sample_size, sample_length, self.device, save_image=save_img, random=random)
    ...
    def job(self, global_weights, episodeNumber, budget_range, sample_size=SAMPLE_SIZE, sample_length=None, random=False):
        print("starting episode {} on metaAgent {}".format(episodeNumber, self.metaAgentID))
        # set the local weights to the global weight values from the master network
        self.set_weights(global_weights)

        jobResults, metrics = self.singleThreadedJob(episodeNumber, budget_range, sample_size, sample_length, random)
